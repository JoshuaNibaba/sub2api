...
func maskUsageAccountName(value string) string {
...
func (h *UsageHandler) List(c *gin.Context) {
	...
	restrictedViewer := service.IsStaffRole(role) && !service.IsSuperAdminRole(role)
	exactTotal := false
	...
	if restrictedViewer {
		// Restricted administrators may inspect only requests generated by
		// their own account; the super administrator retains the global view.
	...
	for i := range records {
		...
		if restrictedViewer && item != nil {
			if item.Account == nil && records[i].AccountID > 0 {
				item.Account = &dto.AccountSummary{ID: records[i].AccountID, Name: "****"}
			} else if item.Account != nil && !records[i].Account.IsOwnedBy(subject.UserID) {
				item.Account.Name = maskUsageAccountName(item.Account.Name)
			}
		}
		if item != nil {
			out = append(out, *item)
...
func (h *UsageHandler) SearchUsers(c *gin.Context) {
	role, _ := middleware.GetUserRoleFromContext(c)
	if service.IsStaffRole(role) && !service.IsSuperAdminRole(role) {
		subject, ok := middleware.GetAuthSubjectFromContext(c)
		if !ok || h.adminService == nil {
			response.Success(c, []any{})
			return
		}
		user, err := h.adminService.GetUser(c.Request.Context(), subject.UserID)
		if err != nil || user == nil || !strings.Contains(strings.ToLower(user.Email), strings.ToLower(strings.TrimSpace(c.Query("q")))) {
			response.Success(c, []any{})
			return
		}
		response.Success(c, []any{gin.H{"id": user.ID, "email": user.Email, "deleted": user.DeletedAt != nil}})
		return
	}
	keyword := c.Query("q")
	if keyword == "" {
...
func (h *UsageHandler) SearchAPIKeys(c *gin.Context) {
	...
	if userIDStr != "" {
		...
		if userID == 0 || id == userID {
			...
			response.Forbidden(c, "Restricted administrators can only search their own API keys")
			return
		}
	}
