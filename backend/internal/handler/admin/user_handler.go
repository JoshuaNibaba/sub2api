...
type CreateUserRequest struct {
	...
	Role                 string   `json:"role" binding:"omitempty,oneof=super_admin admin user enterprise_user"`
...
func (h *UserHandler) Create(c *gin.Context) {
	...
	if req.Role == service.RoleSuperAdmin || req.Role == service.RoleAdmin {
		if role, ok := middleware.GetUserRoleFromContext(c); ok && !service.HasPermission(role, service.PermissionAdminUsersRoleManage) {
			response.Forbidden(c, "Only a super administrator can create administrator accounts")
			return
		}
...
func (h *UserHandler) Update(c *gin.Context) {
	...
	// 防锁死保护：管理员不能把自己降级为普通用户或大客户。
	if userID == getAdminIDFromContext(c) && req.Role != "" && req.Role != service.RoleSuperAdmin && req.Role != service.RoleAdmin {
		response.BadRequest(c, "cannot demote yourself from admin")
		return
	}

	// 角色字段由编辑表单总是携带，因此只有实际发生角色变化时才要求角色管理权限。
	...
	if req.Role != "" {
		...
		if req.Role != target.Role {
			if role, ok := middleware.GetUserRoleFromContext(c); ok && !service.HasPermission(role, service.PermissionAdminUsersRoleManage) {
				response.Forbidden(c, "Only a super administrator can change user roles")
				return
			}
		}
	}
