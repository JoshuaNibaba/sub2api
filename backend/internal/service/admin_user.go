...
func normalizeUserRole(role, fallback string) (string, error) {
	...
	switch role {
	case RoleSuperAdmin, RoleAdmin, RoleUser, RoleEnterpriseUser:
		return role, nil
	default:
		return "", fmt.Errorf("invalid role: %q (must be one of %s, %s, %s, %s)", role, RoleSuperAdmin, RoleAdmin, RoleUser, RoleEnterpriseUser)
	}
}
...
// ensureNotLastSuperAdmin 降级超级管理员前确认系统中仍存在其他超级管理员，防止零超管锁死。
